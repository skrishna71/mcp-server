...
const servers = ExpressHttpStreamableMcpServer({
    ...
    server.tool("getProfile", "get the traits of a user or account", getprofileSchema, async ({ collection, id }) => {
    ...
    server.tool("listEvents", "List events for a user or account", listEventsSchema, async ({ collection, id, since, limit, cursor }) => {
        const baseUrl = `https://profiles.segment.com/v1/spaces/${spaceId}/collections/${collection}/profiles/user_id:${id}`;
        try {
            let para = ``;
    ...
    server.tool("searchEvents", " Client-side filter on listEvents output; narrows by event name and time window ", searchEventsSchema, async ({ id, name, after, before }) => {
        ...
        try {
            const filteredEvents = cachedEvents.data.filter((event) => {
    ...
    server.tool("profileSnapshot", " pulls traits and last n days of events and returns { traits, events } in one call (reduces round-trips)", profileSnapshotSchema, async ({ collection, id, lastDays }) => {
        const baseUrl = `https://profiles.segment.com/v1/spaces/${spaceId}/collections/${collection}/profiles/user_id:${id}`;
